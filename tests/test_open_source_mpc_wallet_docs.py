...
class TestMpcWalletDocCompactOkfFrontmatter:
    def test_has_only_the_six_required_okf_fields(self):
        frontmatter = _frontmatter_block(_read(MPC_DOC))
        keys = re.findall(r"(?m)^([a-z_]+):", frontmatter)
        assert keys == ["okf_version", "type", "title", "created", "status", "language"]

    def test_expanded_metadata_fields_are_absent(self):
        frontmatter = _frontmatter_block(_read(MPC_DOC))
        for removed_key in ("timestamp", "topics", "description", "resource", "sources", "generated", "verified", "stale_after"):
            assert not re.search(rf"(?m)^{removed_key}:", frontmatter)
