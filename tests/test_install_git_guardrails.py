...
class TestStaticAnalysisSteps:
    @pytest.mark.parametrize(
        ("returncodes", "expected_code", "expected_commands"),
        [
            ([17], 17, [["uv", "run", "ruff", "check", "src/"]]),
            (
                [0, 23],
                23,
                [
    ...
    def test_failing_check_short_circuits_and_propagates_exit_code(
        self,
        igg_module,
        tmp_path,
        monkeypatch,
        ...
        """Stop after the first failed quality check and preserve its exit code."""
        _isolate_repo_root(monkeypatch, igg_module, tmp_path)
        (tmp_path / "tools").mkdir(parents=True)
        fake_run = _RecordingSubprocessRun(returncodes=returncodes)
        monkeypatch.setattr(subprocess, "run", fake_run)

        result = igg_module.run_pre_commit_checks()

        assert result == expected_code
        assert [call["args"] for call in fake_run.calls] == expected_commands
        assert all(call["cwd"] == tmp_path for call in fake_run.calls)
