...
class TestSummaryIndexReflectsOkfGuideAddition:
    def test_summary_frontmatter_left_in_legacy_tool_generated_style(self):
        # SUMMARY.md is regenerated by tools/generate_summary.py, which still
        # emits the original OKF v0.1-style, double-quoted frontmatter; this
        # PR intentionally left that generator (and thus SUMMARY.md's own
        # frontmatter block) untouched, only regenerating its body content.
        frontmatter = _frontmatter_block(_read(SUMMARY))
        assert 'okf_version: "0.2"' in frontmatter
        assert 'type: "summary"' in frontmatter
        assert 'title: "Documentation Index & Navigation Summary"' in frontmatter
