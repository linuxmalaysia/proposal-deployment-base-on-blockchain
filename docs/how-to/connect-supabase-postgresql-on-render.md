...
3. **Database Migration Pipeline & Auto-Deployment:** Project schema DDL (`docs/schema.sql`) is automatically checked and created on application startup via FastAPI lifespan context (`auto_check_and_build_schema`), or manually triggered via administrative endpoint (`/api/init-db`) to ensure database tables are deployed seamlessly on Render.com without destroying existing data.
4. **Interactive Connection Status Page:** Access the built-in Database Diagnostic Page at `/db-status` (or `/api/db-status`) to inspect real-time connection status, latency metrics, in-memory TTL caching layer status, and masked configuration flags (`DATABASE_URL_CONFIGURED`, `SUPABASE_SECRET_KEY_CONFIGURED`), along with information_schema table verification.
5. **Key Management & Plural Environment Variable Parsing:** The application adapter parses both singular (`SUPABASE_SECRET_KEY`, `SUPABASE_PUBLISHABLE_KEY`) and plural (`SUPABASE_SECRET_KEYS`, `SUPABASE_PUBLISHABLE_KEYS`) environment variables, supporting plain strings, JSON objects, and JSON arrays.
